...
const WeddingVideoGraph = mongoose.model('WeddingVideoGraph');

exports.create = (req, res) => {
    // var role = req.type;
    // const car = new Car(req.body);
    WeddingVideoGraph.findOne({email:req.body.email}).then(bridal =>{
        if(bridal){
            return res.status(400).json({email:'WeddingVideoGraph already exists'});
        }else{
           const bridal = new WeddingVideoGraph(req.body);
...
exports.update = (req,res) => {
    // role = req.type;
    userId = req.userId;
    WeddingVideoGraph.findById(req.params.id, function(err, car) {
            if(role == 2){
